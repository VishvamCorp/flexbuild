...
define repo-mngr
	for tree in $2; do \
	    ...
	    if [ -n "$$repourl" ] && [ -d $$tree -o -L $$tree ]; then \
		    ...
		    git clone --recurse-submodules --shallow-submodules --depth 1 --branch $$tag $$repourl $$tree $(LOG_MUTE) && \
		    cd $$tree && git checkout -B $$tag $(LOG_MUTE) && cd - $(LOG_MUTE); \
		    ...
		    git clone --recurse-submodules --shallow-submodules --depth 1 $$repourl $$tree -b $$branch $(LOG_MUTE); \
		else \
